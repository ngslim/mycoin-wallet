        ...
        this.chain.forEach(block => {
            block.data.forEach(transaction => {
                if (transaction.from === address) {
                    ...
                    balance -= parseFloat(transaction.gas);
                }
...
class Transaction { 
    ...
    static isValid(tx, chain) { 
        console.log(tx.from);
        console.log(tx.to);
        console.log(ec.keyFromPublic(tx.from, "hex").verify(SHA256(tx.from + tx.to + tx.amount + tx.gas), tx.signature));
        return ( 
            tx.from && 
            tx.to && 
...
module.exports = { Block, Transaction, Blockchain, MyChain, MINT_KEY_PAIR };